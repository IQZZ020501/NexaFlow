...
- `deploy/docker-compose.yml` — 全栈编排：PostgreSQL 17/Redis 7/Qdrant/API/内嵌 Beat 的 Celery worker/Next.js 前端；三个应用服务共用一个可从镜像仓库拉取的应用镜像
...
- `deploy/dockerfiles/app.Dockerfile` — `runtime` 构建业务应用，`sandbox-runtime` 单独构建 Python/Node 执行镜像；业务镜像不含任务运行器或沙箱源码
