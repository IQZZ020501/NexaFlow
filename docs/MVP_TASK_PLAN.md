...
- 工具先做 HTTP 工具；搜索、数据库查询、代码执行等内置工具先不做。
...
| 工具调用 | 调用已授权 HTTP 工具，做参数校验、超时和日志 | MVP-040、MVP-041、MVP-042、MVP-043 |
...
| MVP-030 | Agent | `agent` CRUD，绑定 prompt 和模型 | Agent CRUD API；prompt/model 必填校验 | Agent 列表/创建/编辑 UI | 待实现 |
| MVP-031 | Agent | Agent 绑定知识库 | 绑定/解绑 API；租户范围查询检查 | Agent 编辑页能绑定/解绑知识库 | 待实现 |
| MVP-032 | Agent | 对话 session 和 message 存储 | session/message API 持久化上下文 | Chat UI 能创建 session 并渲染消息 | 待实现 |
| MVP-033 | Agent | RAG 问答闭环 | Agent 判断是否检索知识库，改写查询，检索 chunk，再把证据发给模型适配层 | Chat UI 显示答案和引用 | 待实现 |
...
| MVP-043 | 工具 | Agent 绑定并调用 HTTP 工具 | Agent 只能选择已授权工具，并通过工具层执行 | Chat UI 显示工具调用进度和结果摘要 | 待实现 |
