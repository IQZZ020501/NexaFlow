...
Agent 业务领域与运行时：Agent CRUD、知识库/工具绑定与权限校验；基于 LangGraph 的多轮工具调用执行（计划 → 检索/工具调用 → 结果 → 终止条件），事件流式输出，以及跨模型供应商的会话记忆压缩与用量记录。
...
- 每个 Run 都属于一个 `conversation_id`；同一工作区、Agent、用户、会话最多只有一个活动 Run。未传会话 ID 的旧客户端复用最近会话，前端把当前会话写入 URL，并可显式开始新会话。
