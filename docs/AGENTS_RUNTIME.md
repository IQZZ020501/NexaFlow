...
- AgentSession 把输入消费、CapabilityRegistry、轮次间 ContextManager 和受信任扩展生命周期组合成 harness。已绑定工具默认全部启用以兼容现有行为，模型可 search_tools/activate_tools 管理后续轮次集合；Skills 初始仅目录，通过 load_skill 读取固定版本正文。管理工具只改变内存/下一 checkpoint，不创建第二个动作账本，真实动作仍使用原审批/幂等策略。
...
- Run 创建时冻结整次执行时长、模型轮次和工具调用次数三项执行预算；旧的 `max_model_tokens` 字段继续写入快照以兼容历史 Run，但不再作为 Agent 累计模型 token 门禁。总截止时间只在第一次成功 claim 时写入，worker 重试、租约接管和 checkpoint 恢复不会刷新；人工审批或输入等待时间会显式加回截止时间。每次工具调用仍有独立硬超时，达到最后一轮时不再向模型暴露工具。工具预算耗尽时，所有能力一致撤下工具 schema，允许模型用已有结果完成答案；超预算批次在执行前拒绝。模型供应商的上下文窗口和单次输出限制仍然生效。
...
- 兼容性：数据库及 Run API 的 `grounding_status` / `grounding_meta` 暂留以读取历史数据，新运行固定为 `skipped` / `{}`；旧核验事件不再显示或投影。Skill 定义中旧 `retrieval`、`stop` 与 grounding 评估字段只为版本快照兼容保留，不控制运行或注入提示词。Skill 的通用执行预算、工具/知识绑定、权限与审批仍生效。
