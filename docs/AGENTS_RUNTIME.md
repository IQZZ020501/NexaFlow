...
api/agents.py → application/agents.py（公开用例门面）
             ...
             → application/agent_tools.py + agent_tool_runtime.py（ToolSnapshot → canonical ToolInvocation）
             → shareddomain/agents/services（CRUD/绑定/权限）
             → shareddomain/agents/runtime（LangGraph 状态图执行）
             → capabilities/{llm,rag} + ports/tool_runtime（模型、检索与统一工具端口）
...
- `backend/app/shareddomain/agents/runtime/tools.py` — LangChain StructuredTool 通用适配、参数 schema 校验、暂停/uncertain/busy 信号与元数据读取
...
- `backend/app/application/agent_tools.py` — 知识检索工具与统一 ToolSnapshot 的模型工具包装；保留 legacy MCP 兼容构造
- `backend/app/application/agent_tool_runtime.py` — builtin/Python/MCP 共用 ToolInvocation 的入队、执行、审批暂停与 uncertain 映射
- `backend/app/application/agent_memory.py` — Agent 对话记忆：按 `conversation_id` 读取成功轮次、估算 token 预算、生成持久摘要并保留最近轮次

## 相关测试

- `backend/tests/agents.py` — Agent 端到端测试：检索策略、租约接管、断线、统一 Tool 固定版本、MCP 审批/只读策略、uncertain、预算与安全边界
