 ...
 * Vendored by NexaFlow from open-kimi-ppt-skill and adapted for JSON-only,
 * network-disabled fixed Skill execution. See LICENSE and NOTICE.
 *
 * What this does:
...
// NexaFlow emits JSON, which is a YAML subset. Parse it without installing a
// package. The fixed Skill never accepts caller-supplied project files.
...
async function resolveImages(pptd, projectRoot) {
  ...
  await Promise.all(
    srcs.map(async (src, idx) => {
      ...
      if (/^https?:\/\//i.test(src)) {
        throw new Error(`Remote images are disabled in NexaFlow PPTD exports: ${src}`);
      }
      if (/^data:/i.test(src)) {
