...
type KnowledgeBaseEditForm = KnowledgeBaseForm & {
  id: string
}

type KnowledgeBasePermissionForm = {
  knowledgeBase: KnowledgeBase
  userId: string
  permission: "view" | "edit"
}

type KnowledgeBaseDetailTab = "documents" | "questions" | "hit-test" | "settings"
...
function KnowledgeBasePage({
  ...
  const [editForm, setEditForm] =
    React.useState<KnowledgeBaseEditForm | null>(null)
  ...
  async function handleCreate(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError(null)
    try {
      const knowledgeBase = await createKnowledgeBase(token, selectedWorkspaceId, form)
  ...
  async function handleToggleStatus(knowledgeBase: KnowledgeBase) {
    ...
    try {
      updateKnowledgeBaseInList(
        await updateKnowledgeBase(token, selectedWorkspaceId, knowledgeBase.id, {
          status: nextStatus,
        })
      )
      onNotify(
  ...
  async function handleOpenPermissions(knowledgeBase: KnowledgeBase) {
    ...
    try {
      const [members, grants] = await Promise.all([
        ...
        listKnowledgeBasePermissions(token, selectedWorkspaceId, knowledgeBase.id),
      ])
  ...
  if (selectedKnowledgeBase) {
    return (
      <>
              ...
              {detailTabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    className={cn(
                      "flex h-10 shrink-0 items-center gap-2 rounded-md px-3 text-sm font-medium text-muted-foreground outline-none transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring lg:w-full",
            ...
            {activeDetailTab === "settings" ? (
              <div className="max-w-3xl p-4 lg:p-5">
                ...
                <div className="mt-4 rounded-lg border p-4">
                  <div className="flex flex-wrap gap-2">
                    <PermissionBadge permission={selectedKnowledgeBase.permission} />
        ...
        {renderKnowledgeBaseDialogs()}
      </>
    )
  }

  function renderKnowledgeBaseDialogs() {
            ...
            <div className="flex flex-wrap gap-3">
              {knowledgeBases.map((knowledgeBase) => (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  className="flex min-h-44 w-full min-w-0 cursor-pointer flex-col justify-between rounded-lg border bg-background p-3 shadow-sm outline-none transition-colors hover:bg-muted/40 focus-visible:ring-2 focus-visible:ring-ring sm:w-[19rem]"
