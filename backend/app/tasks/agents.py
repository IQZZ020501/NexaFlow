...
def recover_agent_runs_job() -> None:
    settings = Settings.from_env(require_bootstrap=False)
    configure_task_worker(settings)
    run_ids = asyncio.run(list_recoverable_agent_run_ids(settings))
    for run_id in run_ids:
        run_agent_job.apply_async(args=(run_id,))
...
async def enqueue_agent_run(run_id: str, settings: Settings) -> None:
    ...
    try:
        await asyncio.to_thread(run_agent_job.apply_async, args=(run_id,))
    except Exception as exc:
        log_error(
            logger,
