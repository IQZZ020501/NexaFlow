WITH RECURSIVE active_edges AS MATERIALIZED (
    ...
    SELECT entity_path, claim_path, depth, quality_sum
    FROM walk
    WHERE current_entity_id = :target_entity_id
      AND depth > 0
...
ORDER BY paths.depth ASC NULLS LAST, paths.quality_sum DESC, paths.claim_path ASC
