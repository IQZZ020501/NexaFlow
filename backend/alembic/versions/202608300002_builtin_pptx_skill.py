"""Register the built-in PPTX Skill as a selectable Tool contract."""
...
def _assert_downgrade_safe(
    ...
    for table_name, columns in (
        ...
        if table is not None and bind.scalar(
            ...
            raise RuntimeError("Cannot remove the PPTX Skill Tool while it is referenced.")
...
def downgrade() -> None:
    ...
    workspace_ids = bind.execute(
        ...
        .where(tools.c.stable_key == "skill_pptx")
        .distinct()
    ).scalars()

    for workspace_id in workspace_ids:
        desired_tool, _, _ = _desired(workspace_id)
