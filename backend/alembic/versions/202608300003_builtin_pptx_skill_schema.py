"""Refresh the built-in PPTX Skill contract to the current catalog definition."""
...
down_revision: str | None = "202608300002"
...
def _desired(workspace_id: str):
    ...
    return build_skill_artifact_tool(workspace_id, "pptx")
...
def upgrade() -> None:
    """Re-sync every workspace's PPTX Skill tool to the current catalog.

    Tool versions are content-addressed (stable_catalog_id over the
    definition hash), so a schema change produces a new version row; point
    each workspace's current_version_id at it so validation accepts the
    layouts the renderer supports. Idempotent: workspaces already on the
    current definition are unchanged.
    """
    bind = op.get_bind()
    tables = _tables(bind)
    ...
    for workspace_id in workspace_ids:
        ...
        if bind.scalar(sa.select(tools.c.id).where(tools.c.id == desired_tool.id)) is None:
        ...
        if bind.scalar(
            ...
            # Version rows are immutable and (tool_id, revision) is unique;
            # the catalog builder always emits revision=1, so a refreshed
            # definition needs the next revision for this tool.
...
def downgrade() -> None:
    """No-op: the previous content-addressed version rows are retained.

    Existing tool, version, and policy rows are untouched by upgrade; only
    current_version_id moves forward. Restoring the prior pointer would
    require persisting pre-upgrade state, and the old version rows remain
    queryable for historical snapshots, so the tool stays on the newer
