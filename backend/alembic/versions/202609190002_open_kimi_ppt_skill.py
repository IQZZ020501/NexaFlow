"""Refresh the built-in PPTX Skill to the offline PPTD renderer contract."""
...
def upgrade() -> None:
    """Point every workspace's built-in PPTX Tool at the PPTD definition."""
    bind = op.get_bind()
    tables = _tables(bind)
    if len(tables) != 4:
        return
    ...
    workspace_ids = bind.execute(
        sa.select(sources.c.workspace_id).where(sources.c.kind == "builtin").distinct()
    ).scalars()
...
def downgrade() -> None:
    """Keep immutable Tool versions and invocation history intact."""
