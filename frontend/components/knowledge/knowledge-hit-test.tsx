...
export function KnowledgeHitTest({
  ...
  const [maxDistance, setMaxDistance] = React.useState(2)
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      setResult(
        await inspectKnowledgeBase(token, workspaceId, knowledgeBaseId, {
          query,
          limit: queryLimit,
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            >
              ...
              <select
                ...
                value={searchMode}
                onChange={(event) =>
                  setSearchMode(event.target.value as KnowledgeSearchMode)
                }
                ...
                <article key={hit.chunk_id} className="p-4">
                  <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
                    ...
                    <span>
                      {t("{label}：{value}", {
                        label: t("余弦距离"),
                        value: formatNumber(hit.distance, 4),
                      })}
                    </span>
                  </div>
